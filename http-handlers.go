...
// @Param search query string false "Search in email, username, name"
...
// @Accept json
// @Produce json
// @Param request body services.CreateTaskRequest true "Task data"
...
// @Produce json
// @Param id path int true "Task ID"
// @Param request body services.UpdateTaskRequest true "Task update data"
// @Success 200 {object} entities.Task
// @Failure 400 {object} ErrorResponse
// @Failure 404 {object} ErrorResponse
// @Security BearerAuth
...
func (h *TaskHandler) UpdateTask(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("Update task failed", "error", err, "task_id", taskID, "user_id", userID)
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
...
// @Produce json
// @Param id path int true "Task ID"
// @Param request body AssignTaskRequest true "Assignment data"
// @Success 200 {object} MessageResponse
// @Failure 400 {object} ErrorResponse
// @Failure 404 {object} ErrorResponse
// @Security BearerAuth
...
func (h *TaskHandler) AssignTask(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("Assign task failed", "error", err, "task_id", taskID, "assignee_id", req.AssigneeID)
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}

	return c.JSON(http.StatusOK, MessageResponse{Message: "Task assigned successfully"})
}
...
func (h *TaskHandler) StartTask(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("Start task failed", "error", err, "task_id", taskID, "user_id", userID)
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}

	return c.JSON(http.StatusOK, MessageResponse{Message: "Task started successfully"})
}

// CompleteTask godoc
// @Summary Complete task
// @Description Mark task as completed with actual hours
// @Tags tasks
// @Accept json
// @Produce json
// @Param id path int true "Task ID"
// @Param request body CompleteTaskRequest true "Completion data"
// @Success 200 {object} MessageResponse
// @Failure 400 {object} ErrorResponse
// @Failure 404 {object} ErrorResponse
// @Security BearerAuth
...
func (h *TaskHandler) CompleteTask(c echo.Context) error {
	...
	err = h.taskService.CompleteTask(c.Request().Context(), taskID, req.ActualHours, userID)
	if err != nil {
		h.logger.Error("Complete task failed", "error", err, "task_id", taskID, "user_id", userID)
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}

	return c.JSON(http.StatusOK, MessageResponse{Message: "Task completed successfully"})
}
...
func (h *TaskHandler) ListTasks(c echo.Context) error {
	...
	if priority := c.QueryParam("priority"); priority != "" {
		taskPriority := entities.Priority(priority)
...
type CompleteTaskRequest struct {
	ActualHours float64 `json:"actual_hours" validate:"required,min=0"`
}
